...
def preprocess_geoloc_data(geoloc_cris,geoloc_viirs,selected_cris_fovs):
    ...
    #cris_fov_position = np.array([ vector for index, vector in enumerate(zip( array_1d(geoloc_cris.longs),
    #                                                         array_1d(geoloc_cris.lats) ))  if selected_cris_fovs[index]] ) 
    ...
    for i,cris_fov in enumerate(cris_fov_position):
        ...
        if selected_cris_fovs[i]:
            ...
            #for x in np.intersect1d(lon_ind,lat_ind):
                        #print(transform_index(x,viirs_shape))
                        #print(geoloc_viirs.cloudmask.mask)
            ...
            #print('size indices[{}]: {}'.format(i,len(indices[i]))) 
...
def get_collocation(geoloc_cris, geoloc_viirs, selected_cris_fovs):
    ...
    indices = preprocess_geoloc_data(geoloc_cris,geoloc_viirs, selected_cris_fovs)
...
def test(cris_file_path,viirs_file_path, selected_cris_fovs):
    ...
    indices = get_collocation(geoloc_cris, geoloc_viirs, selected_cris_fovs)
