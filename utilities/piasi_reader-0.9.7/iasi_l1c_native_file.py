...
class IasiL1cNativeFile(object):
    ...
    def get_azimuth_angles(self):
        ...
        if not self.__data_read:
            self.read_mdrs()
        mdrs = self.get_mdrs()
        azimuth_angles_list = [mdr.GGeoSondAnglesSUN[1,:].T for mdr in mdrs]
    ...
    def save_zenith_angles(self,
        ...
        return self.__save_data(self.get_zenith_angles(),
                                ...
                                data_type,
                                shape)
    def save_azimuth_angles(self,
                       output_dir = '.',
                       file_name = 'iasi_azimuth',
                       data_type = None,
                       shape = None):
        return self.__save_data(self.get_azimuth_angles(),
                                output_dir,
                                file_name,
                                data_type,
                                shape)
