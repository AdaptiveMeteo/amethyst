...
from numpy import diag, identity, array, allclose, dot, sqrt
...
class CrisObservationError(object):
    def __init__(self, data_filename):
        ...
        #PaoloA 20062018
        self.__inv_sqrt_obs_err_diag = 1/sqrt(self.__obs_err_diag)
    ...
    #PaoloA 20062019
    @property
    def inv_sqrt_obs_err(self):
        return diag(self.__inv_sqrt_obs_err_diag)
    #PaoloA 20062019 end 
    ...
    #PaoloA 20062019 begin
    def inv_sqrt_obs_err_dot(self, M):
        if len(M.shape) == 1:
            return self.__inv_sqrt_obs_err_diag * M
        elif len(M.shape) == 2:
            inv_sqrt_obs_err_size = self.__inv_sqrt_obs_err_diag.shape[0]
            inv_sqrt_obs_err = self.__inv_sqrt_obs_err_diag.reshape((inv_sqrt_obs_err_size,1))
            return inv_sqrt_obs_err * M
        else:
            return dot(self.inv_sqrt_obs_err, M)

    def dot_inv_sqrt_obs_err(self, M):
        return self.inv_sqrt_obs_err_dot(M.T).T
    #PaoloA 20062019 end 
