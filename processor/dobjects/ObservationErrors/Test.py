...
from numpy import array, allclose, abs, sum, dot, sqrt
...
class CrisObservationError(object):
    ...
    def __init__(self, data_filename):
        ...
        with Dataset(data_filename, mode='r') as df:
            ...
            self.__svd = CrisSVD(U,D,V)

            #PaoloA 13082020
            self.__inv_sqrt_obs_err = dot(U[:, :len(D)] * 1/sqrt(D), V)
            #PaoloA 14082020
            #self.__inv_sqrt_obs_err = dot(U[:, :20] * 1/sqrt(D[:20]), V[:20, :])
    ...
    def inv_sqrt_obs_err(self):
        return self.__inv_sqrt_obs_err
    #PaoloA 13082020 end 
