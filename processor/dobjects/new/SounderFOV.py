...
class SounderFOV(object):
    ...
    def __init__(self, xmlconf):
        ...
        """
        self.df = Dataset(xmlconf.instrumentFOVFile, mode='r')
        ...
        self.fov_type = xmlconf.instrumentName.upper()
        
        # Check if the right channel list file is given in configuration.xml
        if self.fov_type.lower() not in xmlconf.instrumentChannelListFile:
            raise ValueError("Wrong channel list file for instrument {}.".format(self.fov_type))
            
        self.indx =  np.loadtxt(xmlconf.instrumentChannelListFile, dtype=np.int32) - 1  # Read Channel Indices
