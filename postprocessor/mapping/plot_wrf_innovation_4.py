...
parser.add_argument('-w1', 
                    ...
                    help    = 'WRF 1 output path')
parser.add_argument('-w2', 
                    '--wrf_path_2', 
                    type    =str, 
                    required=True,
                    default ='/galaxy/data/hawaii/cris/20201126_235326/wrfda/', 
                    help    = 'WRF 2 output path')
...
fg_file    = args.wrf_path_1 + "/wrfvar_output"
post_file  = args.wrf_path_2 + "/wrfvar_output"
...
for level in [1,10,20]:
    title = "T differences (RUN2-RUN1) Level {}".format(level)
    outfile = '{}/T_Differences_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
...
for level in [1,10,20]:
    title = "$\Theta$ differences (RUN2-RUN1) Level {}".format(level)
    outfile = '{}/THETA_Differences_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
    vmax, vmin = dif[level-1,:,:].max(), dif[level-1,:,:].min()
    plot_colormesh(lons,lats,dif[level-1,:,:],projection,title,
...
dif *= 1e3
for level in [1,10,20]:        
    title = "QVAPOR Differences (RUN2-RUN1) Level {}".format(level)
    outfile = '{}/Q_differences_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
    vmax, vmin = dif[level-1,:,:].max(), dif[level-1,:,:].min()
...
for level in [1,10,20]:        
    title = "RH Differences (RUN2-RUN1) Level {}".format(level)
    outfile = '{}/RH_differences_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
    vmax, vmin = dif[level-1,:,:].max(), dif[level-1,:,:].min()
...
rh = getvar(fg,'rh')
for level in [1,10,20]:
    title = "RH(RUN1) Level {}".format(level)
    outfile = '{}/RH_RUN1_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
...
for level in [1,10,20]:
    title = "PV(RUN1) Level {}".format(level)
    outfile = '{}/PV_RUN1_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
...
for level in [1,10,20]:
    title = "PV(RUN2) Level {}".format(level)
    outfile = '{}/PV_RUN2_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
    vmax, vmin = pv[level-1,:,:].max(), pv[level-1,:,:].min()
    plot_colormesh(lons,lats,pv[level-1,:,:],projection, title,outfile, args.world_area,
                   clabel = 'Potential Vorticity [$m^2 s^{-1}K kg^{-1}$]',vmin=vmin,vmax=vmax, tr_file = args.tr)
...
for level in [1,10,20]:
    title = "PV Differences (RUN2-RUN1) Level {}".format(level)
    outfile = '{}/PV_differences_L{:02d}_{}.png'.format(args.output,level,assimilationwindow_title)
    vmax, vmin = dif[level-1,:,:].max(), dif[level-1,:,:].min()
    plot_colormesh(lons,lats,dif[level-1,:,:],projection, title,outfile, args.world_area,
