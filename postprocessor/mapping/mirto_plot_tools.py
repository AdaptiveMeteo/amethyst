...
class Tiff_Image(object):
    ...
    def __init__(self,overpass_path):
        ...
        if abs((self.overpass_date - ref_tif_date).total_seconds()//60) < 60*TIF_N_HOURS:
            tif_file     = [ file for file in os.listdir(overpass_path) if 'tif' in file and 'dnb' in file ]
        else:
            tif_file     = [ file for file in os.listdir(overpass_path) if 'tif' in file and 'color' in file ]
        if len(tif_file)==0:
                          sys.exit("No Tiff file found")
...
def plot_fovs(tif,fov,results, outfile, world_area, stations = None):
    """
    Plot FOVs on 
    ...
    xmin = fov.longitude.min() 
    xmax = fov.longitude.max() 
    ymin = fov.latitude.min() 
    ymax = fov.latitude.max() 
   
    extent = tif.extent if tif!=None else [xmin, xmax, ymin, ymax ]
    plt.figure(figsize=(15, 15))
    
    if world_area == 'pacific':
        # Plot Map
        m = Basemap( projection='cyl',
                llcrnrlon=extent[0], # min x
                llcrnrlat=extent[2], # min y
                urcrnrlon=extent[1], # max x
                urcrnrlat=extent[3], # max y
                resolution='h')
    else:
        lon_0 = 180
        lat_0 = 90
        ...
        m.fillcontinents(color='palegoldenrod',lake_color='aqua')
    ...
    for fov_index, xlon in enumerate(x):
        ...
        # Draw the patch
        projx,projy = m(xlon,y[fov_index])
        e = Ellipse((projx,projy),L1,L2,angle,facecolor="red",edgecolor='red',zorder=1)
        plt.scatter(projx,projy,marker='x',s=2,color='red')
        plt.gca().add_patch(e)
    
    if stations != None:
        for point in zip(stations.LON,stations.LAT):
            projx,projy = m(point[0],point[1])
            plt.scatter(projx,projy,marker='s',c='blue',s=1,zorder=1)
            # Draw station patch

            e = Ellipse((projx,projy),1e5,1e5,0,facecolor="none",edgecolor='blue',zorder=1)
            plt.gca().add_patch(e)
...
def read_overpass(overpass_path, read_tif = True):
...
def prepare_parser():
    ...
    parser.add_argument('--plot_stations',type=bool,
                        help='Boolean for plotting stations whith overpass')
    parser.add_argument('--plot_tif',type=bool,default=False,
                        help='Boolean for plotting tif')
    parser.add_argument('-wa','--world_area',type=str,choices=["pacific","arctic"],default="pacific",
                        help='World Area')
...
def main():
      ...
      if argv.mode == 'plot_overpass':
          ...
          if argv.input[-1]=='/':
              argv.input = argv.input[:-1]
              
          res, fov, tif = read_overpass(argv.input,read_tif = argv.plot_tif) # Read geoTIFF, fov.nc and results.nc
          outfile = "/".join([ argv.outdir, 'overpass_plot_'+os.path.basename(argv.input) +'.png'  ])
          tif = tif 
          
          if argv.plot_stations:
              from validation.read_stations_list import stations_list
              outfile = outfile.replace('.png','_with_stations.png')
              plot_fovs( tif, fov, res, outfile, argv.world_area, stations=stations_list()  )
          else:
              plot_fovs(tif, fov, res, outfile, argv.world_area ) 
