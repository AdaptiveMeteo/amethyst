...
def WRF_SaveResults_nc(out_delta, wrf_data, sonde,
                       title, dists, workdir):
    ...
    n_fovs        = ncfile.createDimension('n_points',len(out_delta))
    ...
    nc_delta_wv = ncfile.createVariable('delta_water_vapour',np.float32,('n_points','scalar'))
    nc_delta_rh = ncfile.createVariable('delta_rh',np.float32,('n_points','scalar'))
    nc_delta_temp = ncfile.createVariable('delta_temp',np.float32,('n_points','scalar'))

    nc_delta_profile_wv = ncfile.createVariable('delta_profile_water_vapour',np.float32,('n_points','n_lev'))
    nc_delta_profile_rh = ncfile.createVariable('delta_profile_rh',np.float32,('n_points','n_lev'))
    nc_delta_profile_temp = ncfile.createVariable('delta_profile_temp',np.float32,('n_points','n_lev'))
    ...
    for index, delta in enumerate(out_delta):
        ...
        # Write Prior RMS
        nc_delta_temp[index,:] = delta[0].temp
        nc_delta_rh[index,:]   = delta[0].rh
        nc_delta_wv[index,:]   = delta[0].water_vapour
    
        # Write Prior RMS Profile
        nc_delta_profile_temp[index,:] = delta[1].temp
        nc_delta_profile_rh[index,:]   = delta[1].rh
        nc_delta_profile_wv[index,:]   = delta[1].water_vapour
        ...
        if  np.isnan(delta[0].temp) or np.isinf(delta[0].rh) \
            or np.isnan(delta[0].water_vapour) or np.isinf(delta[0].water_vapour)   :
...
def WRF_Validation(wrf_data, sonde, station, 
    ...
    wrf_delta = ValidationDataset( 
                    np.mean(int_sonde.temp - wrf_profile.temp),
                    np.mean(int_sonde.rh   - wrf_profile.rh),
                    np.mean(int_sonde.water_vapour - wrf_profile.water_vapour)
                           )
    wrf_delta_profile = ValidationDataset( 
        int_sonde.temp - wrf_profile.temp,
        int_sonde.rh   - wrf_profile.rh,
        int_sonde.water_vapour - wrf_profile.water_vapour 
                          )
    out_delta.append( [wrf_delta,  
                     wrf_delta_profile,
                     sonde.date,
                     index])
...
def generate_statistics_file( outdir, timestep ):
    ...
    delta_temp_profile=agg_nc_file.sonde_temp - agg_nc_file.temp
    delta_rh_profile=agg_nc_file.sonde_rh - agg_nc_file.rh
    delta_wv_profile=agg_nc_file.sonde_water_vapour-agg_nc_file.water_vapour
        
    temp_delta = np.ma.mean(np.ma.mean(delta_temp_profile))
    rh_delta = np.ma.mean(np.ma.mean(delta_rh_profile))
    wv_delta = np.ma.mean(np.ma.mean(delta_wv_profile))
    ...
    nc_delta_wv = ncfile.createVariable('delta_water_vapour',np.float32,('n_cicles','scalar'))
    nc_delta_rh = ncfile.createVariable('delta_rh',np.float32,('n_cicles','scalar'))
    nc_delta_temp = ncfile.createVariable('delta_temp',np.float32,('n_cicles','scalar'))

    nc_delta_profile_wv = ncfile.createVariable('delta_profile_water_vapour',np.float32,('n_cicles','n_lev'))
    nc_delta_profile_rh = ncfile.createVariable('delta_profile_rh',np.float32,('n_cicles','n_lev'))
    nc_delta_profile_temp = ncfile.createVariable('delta_profile_temp',np.float32,('n_cicles','n_lev'))
    ...
    # Write RMS
    nc_delta_temp[:] =  temp_delta
    nc_delta_rh[:]   =  rh_delta
    nc_delta_wv[:]   =  wv_delta

    # Write RMS Profile
    nc_delta_profile_temp[:,:] = delta_temp_profile
    nc_delta_profile_rh[:,:]   = delta_rh_profile
    nc_delta_profile_wv[:,:]   = delta_wv_profile
